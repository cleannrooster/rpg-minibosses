...
public class MinibossEntity extends PathAwareEntity implements Tameable, GeoEntity, Angerable {
    ...
    public AnimatableInstanceCache instanceCache = mod.azure.azurelib.util.AzureLibUtil.createInstanceCache(this);
    @Override
        ...
        if(this.age % 20 == 0 && this.getRandom().nextInt(19) == 0 && this.getWorld() instanceof ServerWorld serverWorld && serverWorld.getPlayers().stream().anyMatch(player -> this.distanceTo(player) < 6 && this.canSee(player))){
            for(MinibossEntity boss : this.getWorld().getEntitiesByType(TypeFilter.instanceOf(MinibossEntity.class),this.getBoundingBox().expand(8), Predicates.alwaysTrue())) {
                if (!boss.notPetrified()) {
                    boss.removeStatusEffect(this.getIntroEffect().value());
    ...
    public void playReleaseParticlesAndSound(){
        if(!this.getWorld().isClient()) {
            net.spell_engine.particle.ParticleHelper.sendBatches(this, net.spell_engine.internals.SpellRegistry.getSpell(Identifier.of(RPGMinibosses.MOD_ID, "pound")).release.particles);
    ...
    public final boolean cannotFollowOwner() {
        return   this.isSitting() || this.hasVehicle()  || this.getOwner() != null && this.getOwner().isSpectator();
    }
    ...
    protected void playStepSound(BlockPos pos, BlockState state) {
        this.playSound(SoundEvents.ITEM_ARMOR_EQUIP_CHAIN,  0.05F, 0.8F);
    ...
    private boolean canTeleportTo(BlockPos pos) {
        PathNodeType pathNodeType = LandPathNodeMaker.getLandNodeType(this.getWorld(), getBlockPos().mutableCopy());
    ...
    public class MinibossMoveConrol extends MoveControl{
        ...
        private boolean isPosWalkable(float x, float z) {
            ...
            if (entityNavigation != null) {
                ...
                BlockPos.Mutable mutable = BlockPos.Mutable.ofFloored(this.entity.getX() + (double)x, (double)this.entity.getBlockY(), this.entity.getZ() + (double)z).mutableCopy();
