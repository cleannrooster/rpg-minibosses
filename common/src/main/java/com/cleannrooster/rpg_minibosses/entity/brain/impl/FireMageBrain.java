...
public class FireMageBrain extends MobBrain {
    ...
    private void fireFireball() {
        ...
        SpellHelper.shootProjectile(entity.getWorld(), entity, entity.getTarget(),
            SpellRegistry.from(entity.getWorld()).getEntry(Identifier.of(RPGMinibosses.CONTENT_NAMESPACE, "fireball")).get(),
        ...
        ParticleHelper.sendBatches(entity,
            SpellRegistry.from(entity.getWorld()).get(Identifier.of(RPGMinibosses.CONTENT_NAMESPACE, "fireball")).release.particles);
    ...
    private void fireVolley() {
        ...
        ((WorldScheduler) entity.getWorld()).schedule(60, () -> {
            if (entity.getTarget() != null && entity.canSee(entity.getTarget())) {
                SoundHelper.playSound(entity.getWorld(), entity, new Sound(SpellEngineSounds.GENERIC_FIRE_RELEASE.id()));
                SpellHelper.shootProjectile(entity.getWorld(), entity, entity.getTarget(),
                    SpellRegistry.from(entity.getWorld()).getEntry(Identifier.of(RPGMinibosses.CONTENT_NAMESPACE, "lesser_fire_volley")).get(),
                    new SpellHelper.ImpactContext().power(SpellPower.getSpellPower(SpellSchools.FIRE, entity)).position(entity.getPos()));
                ParticleHelper.sendBatches(entity,
                    SpellRegistry.from(entity.getWorld()).get(Identifier.of(RPGMinibosses.CONTENT_NAMESPACE, "lesser_fire_volley")).release.particles);
            }
            entity.performing = false;
        });
    }
                ...
                if (entity.getTarget() != null && entity.canSee(entity.getTarget())) {
                    ...
                    for (Entity target : TargetHelper.targetsFromArea(entity, 6, new Spell.Target.Area(), null)) {
                        SpellHelper.performImpacts(entity.getWorld(), entity, target, entity,
                            SpellRegistry.from(entity.getWorld()).getEntry(Identifier.of(RPGMinibosses.CONTENT_NAMESPACE, "fire_nova")).get(),
                            SpellRegistry.from(entity.getWorld()).get(Identifier.of(RPGMinibosses.CONTENT_NAMESPACE, "fire_nova")).impacts,
                            new SpellHelper.ImpactContext().power(SpellPower.getSpellPower(SpellSchools.FIRE, entity)).position(entity.getPos()));
                    }
                    ParticleHelper.sendBatches(entity,
                        SpellRegistry.from(entity.getWorld()).get(Identifier.of(RPGMinibosses.CONTENT_NAMESPACE, "fire_nova")).release.particles);
                }
                entity.performing = false;
            });
        });
